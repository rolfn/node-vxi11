...
function vxiCloseDevice(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    clink.logger.log('prev. Xid:%d     current Xid:%d', clink.xid, newXid);
    clink.socket.end();
  });

  clink.xid = Date.now() >>> 1;
  var a = [];
...
function vxiSend(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    var oldXid = clink.xid;// TODO: XID check!
    ...
    client.once('data', function(data) {
      ...
      var oldXid = clink.xid;
      var newXid = data.readUInt32BE(4);
      clink.logger.log('prev. Xid:%d     current Xid:%d', oldXid, newXid);
