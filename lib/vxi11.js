...
function vxiCloseDevice(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    clink.logger.log('XID (call):%d   XID (reply):%d', clink.xid, newXid);
    clink.socket.end();
  });
  ...
  a.push(clink.linkId);
  var buf = getBuffer(clink, 'DESTROY_LINK call', a);
  client.write(buf);
}
...
function vxiReceive(clink, clbk) {
  ...
  var buf = getBuffer(clink, 'DEVICE_READ call', a);
  client.write(buf);
}
...
function vxiSend(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    mLength = l % 4 ? l + 4 - l % 4 : l;// multiple of 4 Bytes
    ...
    var buf = getBuffer(clink, 'DEVICE_WRITE call', a, tmpbuf);
    client.write(buf);
  });
  ...
  client.on('connect', function() {
    ...
    var buf = getBuffer(clink, 'CREATE_LINK call', a,
...
vxiOpenDevice = function (clink, clbk) {
  ...
  client.on('connect', function() {
    ...
    var buf = getBuffer(clink, 'GETPORT call', a);
    client.write(buf);
  });
}
