...
function vxiCloseDevice(clink, clbk) {
  ...
  client.on('end', function() {
    clink.logger.log('client disconnected (port: %d)', clink.port);
  ...
  a.push(clink.linkId);
  client.write(getBuffer(clink, 'DESTROY_LINK call', a));
}
...
function vxiReceive(clink, clbk) {
  ...
  client.write(getBuffer(clink, 'DEVICE_READ call', a));
}
...
function vxiSend(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    client.write(getBuffer(clink, 'DEVICE_WRITE call', a,
      getAlignedBuffer(clink.command)));
  });

  client.on('connect', function() {
    clink.logger.log('client connected (port: %d)', clink.port);
    ...
    client.write(getBuffer(clink, 'CREATE_LINK call', a,
...
vxiOpenDevice = function (clink, clbk) {
  ...
  var client = net.connect(PMAP_PORT, clink.host);
  client.setNoDelay(true);

  client.on('end', function() {
    clink.logger.log('client disconnected (port: %d)', PMAP_PORT);
    if (typeof clbk == 'function') clbk(clink);
  });
  ...
  client.on('connect', function() {
    clink.logger.log('client connected (port: %d)', PMAP_PORT);
    ...
    client.write(getBuffer(clink, 'GETPORT call', a));
  });
}
