/*
  Rolf Niepraschk, Rolf.Niepraschk@gmx.de, 2015-05-19
*/
...
function rpc(clink, clbk) {
  ...
  var data = new Buffer(0), first = true;

  function getLastFragment(buf) {
    var flen = 0, len = 0, ret = false, idx = buf.length - 4, mask = 1 << 31;
    while (idx > -1) {
      flen = buf.readUInt32BE(idx);
      if ((flen & mask) != 0) {// is the highest bit set?
        flen &= ~mask;// clear the highest bit
        if (flen == len) {
          ret = { idx:idx };
          break;
        }
  ...
  function onData(chunk) {
    var lastFragment = false, buf;
    if (first) {
      first = false;
      data = Buffer.concat([data,chunk]);
      lastFragment = data.readUInt8(0) >= 128;
      clink.logger.log('FIRST CALL' + (lastFragment ? ' (last fragment)' : ''));
    } else {
      lastFragment = getLastFragment(chunk);
      if (lastFragment) {
        buf = chunk.slice(0, lastFragment.idx);
        data = Buffer.concat([data,buf]);
        buf = chunk.slice(lastFragment.idx+4);
        data = Buffer.concat([data,buf]);
      } else {
        data = Buffer.concat([data,chunk]);
      }
      inspectBuffer(clink, 'chunk', chunk.slice(48));
      clink.logger.log('NEXT CALL' + (lastFragment ? ' (last fragment)' : ''));
    }
