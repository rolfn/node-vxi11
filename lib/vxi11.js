/*
  Rolf Niepraschk, Rolf.Niepraschk@gmx.de, 2015-02-20
*/
...
function vxiDeviceRead(clink, clbk) {
  var socket = clink.socket, proc = DEVICE_READ;
  _rpc(clink, proc, function (clink, data, error) {
      clink.error = '';
      if (!error || error == IO_TIMEOUT && clink.readTimeout == 0) {
      // "readTimeout == 0" --> special case for buggy devices
        var len = data.readUInt32BE(LEN_OFFS), b;
        // TODO: check needed!
        if (len > 0) {
          b = data.slice(DATA_OFFS, DATA_OFFS + len);
...
function vxiDeviceWrite(clink, clbk) {
  var socket = clink.socket, proc = DEVICE_WRITE;
  _rpc(clink, proc, function (clink, data, error) {
      clink.error = '';
      if (error) {
        clink.error = getErrorMsg(proc, error);
