...
function vxiReceive(clink, clbk) {
  ...
  client.once('data', function(data) {
    ...
    console.log('DEVICE_READ reply [%d]: %s', data.length, util.inspect(data));
  ...
  console.log('DEVICE_READ call [%d]: %s', buf.length, util.inspect(buf));
...
function vxiSend(clink, cmd, clbk) {
  ...
  client.once('data', function(data) {
    console.log('CREATE_LINK reply [%d]: %s', data.length, util.inspect(data));
    var oldXid = clink.xid.readUInt32BE(0);/// TODO: XID check!
    var newXid = data.readUInt32BE(4);
    ...
    console.log('DEVICE_WRITE call [%d]: %s', buf.length, util.inspect(buf));

    client.once('data', function(data) {
      console.log('DEVICE_WRITE reply [%d]: %s', data.length, util.inspect(data));
